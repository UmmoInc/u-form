...
export default function Enterprising_Feed(data) {
  ...
  const questions = data.data[0].category_info[4].question_info;
  const section_name = data.data[0].title;
  const category_name = data.data[0].category_info[4].title;
  ...
  function increament() {
    ...
    if (questionNumber === questions.length) {
      setClose(true);
      setShowLoader(true);
      router.push("/test/section-one/conventional/questions");
