...
export default function RealisticQuestions(section_info) {
  const [questionNumber, setQuestionNumber] = useState(1);
  const [categoryNumber, setCategoryNumber] = useState(1);

  const sectionTitle = section_info.section_info.title;
  const sectionId = section_info.section_info.section_id;
  const testId = section_info.section_info.test_id;
  const category_info = section_info.section_info.category_info[0];
  ...
  function option_0() {
    const response = {
      ...
      option_id: questionInfo[questionNumber - 1].options[0].id,
      question_id: questionInfo[questionNumber - 1].question_id,
    };
    gradeResponse.push(response);
  }
  ...
  function option_1() {
    const response = {
      category_name: categoryTitle,
      section_name: sectionTitle,
      option_id: questionInfo[questionNumber - 1].options[1].id,
      question_id: questionInfo[questionNumber - 1].question_id,
    };
    gradeResponse.push(response);
  }
  ...
  function increament(params) {
    if (questionNumber === questionInfo.length) {
      router.push("/test/section-one/investigative-questions");
    ...
    <div className="space-y-8">
      <div className="">
        <div className="w-full justify-center flex flex-col items-center space-y-2">
          <p className="text-[#0079B0] font-semibold text-lg tracking-wider">QUESTION</p>
          <p className="text-[#0079B0] font-normal tracking-widest text-lg ">{questionNumber}/{questionInfo.length}</p>
        </div>
      </div>
      <div className=" w-96  bg-white p-4 rounded-3xl  tracking-wider border border-[#006592] border-opacity-25">
        <div>
          <div className="space-y-2 tracking-wider">
            <p className="text-[#8794A1] font-normal tracking-widest">{categoryTitle}</p>
