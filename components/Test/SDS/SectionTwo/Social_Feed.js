...
export default function Social_Feed(data) {
  ...
  const questions = data.data[1].category_info[2].question_info;
  const section_name = data.data[1].title;
  const category_name = data.data[1].category_info[2].title;
  ...
  function increament() {
    ...
    if (questionNumber === questions.length) {
      setClose(true);
      setShowLoader(true);
      router.push("/test/section-two/artistic/questions");
    };
  };
