...
export default function Investigative_Feed(data) {

  //Initializing Index Number
  const [catergoryIndex, setCatergoryIndex] = useState(0)

  //Questions data From API
  const questions = data.data[0].category_info[1].question_info;
  const section_name = data.data[0].title;
  const category_name = data.data[0].category_info[1].title;
  ...
  function increament() {
    ...
    if (questionNumber === questions.length) {
      setClose(true);
      setShowLoader(true);
      router.push("/test/section-one/social/questions");
    };
  };
